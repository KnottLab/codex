...
%     axis([D.frames{i,2}([1 end]) D.frames{i,1}([1 end])])
    ...
    mkdir(['./outputs/3_Segmentation/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),])
    imwrite(tissue_mask,['./outputs/3_Segmentation/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'_1_tissue.tif'],'tif')
...
%     axis([D.frames{i,2}([1 end]) D.frames{i,1}([1 end])])
    ...
    if(isfield(CODEXobj, 'BG1'))
        CODEXobj = rmfield(CODEXobj,'BG1');
    end
    if(isfield(CODEXobj, 'BG2'))
        CODEXobj = rmfield(CODEXobj,'BG2');
    end
    mkdir(['./outputs/3_Segmentation/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),])
    save(['./outputs/3_Segmentation/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'_CODEXobj.mat'],'CODEXobj','-v7.3'); disp(CODEXobj)
    bin_nuclei = uint8(CODEXobj.nuclei_mask>0)*255;
    bin_membrane = uint8(CODEXobj.membrane_mask>0)*255;
    imwrite(bin_nuclei, ['./outputs/3_Segmentation/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'_2_nuclei.tif'])
    imwrite(bin_membrane, ['./outputs/3_Segmentation/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'_2_membrane.tif'])
    writetable(CODEXobj.cells, ['./outputs/3_Segmentation/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'/',CODEXobj.sample_id,'_reg',num2str(CODEXobj.region),'_2_centroids.csv'])
