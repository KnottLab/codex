function im = focus_stack_BBC_gpu(Is)

Is = gpuArray(double(Is)/65535);
...
%********** Compute fmeasure **********
fm = zeros(opts.size,'gpuArray');
for p = 1:P
    fm(:,:,p) = gfocus(Is(:,:,p), opts.nhsize);
end
...
if(sum(isnan(phi(:)))==0)
    ...
    im = uint16(sum(65535*Is.*fm,3)./fmn);
    im = gather(im);
