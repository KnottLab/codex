...
class ConservingLayer(Layer):
    def __init__(self, n_features, n_augm, **kwargs):
        if 'input_shape' not in kwargs and 'input_dim' in kwargs:
            kwargs['input_shape'] = (kwargs.pop('input_dim'),)
        super(ConservingLayer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        input_dim = input_shape[1]
        self.input_spec = InputSpec(dtype=K.floatx(), shape=(None, input_dim))
        self.clusters = self.add_weight(shape=(self.n_features, input_dim), initializer='glorot_uniform', name='clusters')
        if self.initial_weights is not None:
            self.set_weights(self.initial_weights)
            del self.initial_weights
        self.built = True
    ...
    def compute_output_shape(self, input_shape):
        assert input_shape and len(input_shape) == 2
        return input_shape[0], self.n_augm

    def get_config(self):
        config = {'n_augm': self.n_augm}
        base_config = super(ConservingLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class DCEC(object):
    def __init__(self,
        ...
        conserving_layer = ConservingLayer(self.n_clusters, name='conserving')(hidden)
        self.model = Model(inputs=self.cae.input,outputs=[conserving_layer, self.cae.output])
    ...
    def pretrain(self, x, batch_size=256, epochs=200, optimizer='adam', save_dir='results/temp'):
        ...
        #x1w = self.cae.get_layer(index=0).get_weights()[0][:,:,0,:]
        #print(x1w.shape)
        #for i in range(1,x1w.shape[2]+1):
        #    plt.subplot(4,8,i)
        ...
        Y = self.cae.predict(x, batch_size=batch_size, verbose=2)
        print(Y.shape)
        print(type(Y))
        from scipy.io import savemat
        mdic = {"Y": Y, "label": "experiment"}
        savemat(save_dir + '/cae_output.mat', mdic)
    ...
    def fit(self, x, y=None, batch_size=256, epochs=200,maxiter=150, tol=1e-3,
        ...
        print('\n################  Export results  ################\n')
        y_pred = self.y_pred
        np.savetxt(args.save_dir + '/clusters_0.txt', y_pred, delimiter=',')

        features = self.encoder.predict(x)
        print('feature shape =', features.shape)
        np.savetxt(args.save_dir + '/features_0.txt', features, delimiter=',')
        print('\n################  All done  ################\n')
        ...
        from scipy.io import savemat
        mdic = {"Y": Y, "label": "experiment"}
        savemat(save_dir + '/cae_output_2.mat', mdic)
...
if __name__ == "__main__":
    ...
    print('\n**  Encoder summary**\n')
    dcec.encoder.summary()
    plot_model(dcec.encoder, to_file=args.save_dir + '/encoder_model.png', show_shapes=True)
    print('\n################  DCEC model initialized  ################\n')

    print(dcec)
    print(dcec.model)
    print(dcec.cae)
    print(dcec.cae.get_layer(index=0))
    print('\n################  Compiling DCEC  ################')
